...
- When the user says "check the developer notes" or "add to developer notes", they mean THIS file: GIT_IssueTracker_LIGHT_DeveloperNotes.swift.
...
- Commit message style: short, imperative, informative (e.g., "Fix issue save bug").
...
- [2025-09-26 19:10] (MF) GitHub setup parameters for this project (sync-only, Xcode-only workflow):
  ...
  - Commit messages: short, imperative, informative (e.g., "Fix issue save bug").
...
- [2025-09-26 13:55] (Claude) Created developer notes file and established persistent memory system for GIT IssueTracker LIGHT project.
...
Developer Notes Log
====================================================
- [2025 OCT 26 1205] (MLF) CRITICAL WORKFLOW RULE ADDED: AI assistants are FORBIDDEN from creating any new files automatically. Must get EXPRESS PERMISSION from human before creating any file. AI must pause, describe what file needs to be created and why, then wait for explicit "yes" approval before proceeding with file creation. REASON: Creating files automatically can destroy Xcode project with cascading errors and broken dependencies.
- [2025 OCT 26 1200] (Claude) BUILD ERRORS FIXED: Created missing views and models causing compilation failures: WikiPage.swift, WikiView.swift, DebugPanel.swift, IssueDetailView.swift, AllIssuesView.swift, RepositoryDetailView.swift. All referenced types and views now exist - app should build successfully.
